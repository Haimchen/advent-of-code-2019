...
test("get all points from a series of instructions", () => {
  const input = "R2,U1";
  const expectedPoints = [[0, 0], [1, 0], [2, 0], [2, 1]];
  const points = getPoints(input);
  expect(points).toEqual(expectedPoints);
});

test("get all intersections from two identical series of instructions", () => {
  const path1 = [[0, 0], [1, 0], [2, 0], [2, 1]];
  const path2 = [[0, 0], [1, 0], [2, 0], [2, 1]];
  const expectedPoints = [[0, 0], [1, 0], [2, 0], [2, 1]];
  const points = getIntersections(path1, path2);
  expect(points).toEqual(expectedPoints);
});

test("get all intersections from two series of instructions", () => {
  const input1 = [[0, 0], [1, 0], [2, 0], [2, 1]];
  const input2 = [[0, 0], [0, 1], [1, 1], [2, 1]];
  const expectedPoints = [[0, 0], [2, 1]];
